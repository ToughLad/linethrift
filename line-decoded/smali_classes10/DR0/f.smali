.class public final LDR0/f;
.super LDR0/d;
.source "SourceFile"


# annotations
.annotation system Ldalvik/annotation/Signature;
    value = {
        "LDR0/d<",
        "LER0/d;",
        ">;"
    }
.end annotation


# instance fields
.field public final A:LQ01/M1;

.field public final B:[LLv0/h;


# direct methods
.method public constructor <init>(LQ01/M1;LuR0/a;)V
    .locals 3

    invoke-direct {p0, p1, p2}, LDR0/d;-><init>(Ly5/a;LuR0/a;)V

    iput-object p1, p0, LDR0/f;->A:LQ01/M1;

    new-instance p1, LLv0/h;

    sget-object p2, LmQ0/f;->b:Ljava/util/Set;

    const v0, 0x7f0b1800

    const/4 v1, 0x0

    invoke-direct {p1, v0, v1, p2}, LLv0/h;-><init>(IILjava/util/Set;)V

    new-instance v0, LLv0/h;

    sget-object v1, LLv0/h;->d:Ljava/util/EnumSet;

    const v2, 0x7f0b1801

    invoke-direct {v0, v2, p2, v1}, LLv0/h;-><init>(ILjava/util/Set;Ljava/util/Set;)V

    filled-new-array {p1, v0}, [LLv0/h;

    move-result-object p1

    iput-object p1, p0, LDR0/f;->B:[LLv0/h;

    return-void
.end method


# virtual methods
.method public final bridge synthetic A0(LER0/c;)V
    .locals 0

    check-cast p1, LER0/d;

    invoke-virtual {p0, p1}, LDR0/f;->C0(LER0/d;)V

    return-void
.end method

.method public final B0(LER0/c;)V
    .locals 2

    check-cast p1, LER0/d;

    const-string v0, "viewData"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/n;->h(Ljava/lang/Object;Ljava/lang/String;)V

    iget-object p0, p0, LDR0/f;->A:LQ01/M1;

    iget-object v0, p0, LQ01/M1;->e:Landroid/view/View;

    check-cast v0, Landroid/widget/TextView;

    const-string v1, "mainTextViewStretched"

    invoke-static {v0, v1}, Lkotlin/jvm/internal/n;->g(Ljava/lang/Object;Ljava/lang/String;)V

    invoke-virtual {v0}, Landroid/view/View;->getVisibility()I

    move-result v0

    iget-object p1, p1, LER0/d;->b:Ljava/lang/String;

    if-nez v0, :cond_0

    iget-object p0, p0, LQ01/M1;->e:Landroid/view/View;

    check-cast p0, Landroid/widget/TextView;

    invoke-virtual {p0, p1}, Landroid/widget/TextView;->setText(Ljava/lang/CharSequence;)V

    return-void

    :cond_0
    iget-object p0, p0, LQ01/M1;->b:Landroid/widget/TextView;

    invoke-virtual {p0, p1}, Landroid/widget/TextView;->setText(Ljava/lang/CharSequence;)V

    return-void
.end method

.method public final C0(LER0/d;)V
    .locals 6

    iget-object v0, p0, LDR0/d;->y:LuR0/a;

    if-eqz v0, :cond_0

    iget-object v0, v0, LuR0/a;->f:Ljava/util/Map;

    iget v1, p1, LER0/d;->e:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v1

    invoke-interface {v0, v1}, Ljava/util/Map;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/lang/Integer;

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    :goto_0
    const/4 v1, 0x0

    if-eqz v0, :cond_1

    const/4 v0, 0x1

    goto :goto_1

    :cond_1
    move v0, v1

    :goto_1
    iget-object v2, p0, LDR0/f;->A:LQ01/M1;

    iget-object v3, v2, LQ01/M1;->e:Landroid/view/View;

    check-cast v3, Landroid/widget/TextView;

    const/16 v4, 0x8

    if-eqz v0, :cond_2

    move v5, v1

    goto :goto_2

    :cond_2
    move v5, v4

    :goto_2
    invoke-virtual {v3, v5}, Landroid/view/View;->setVisibility(I)V

    iget-object v2, v2, LQ01/M1;->b:Landroid/widget/TextView;

    if-nez v0, :cond_3

    goto :goto_3

    :cond_3
    move v1, v4

    :goto_3
    invoke-virtual {v2, v1}, Landroid/view/View;->setVisibility(I)V

    invoke-super {p0, p1}, LDR0/d;->A0(LER0/c;)V

    return-void
.end method

.method public final bridge synthetic r0(LSP0/d$b;)V
    .locals 0

    check-cast p1, LER0/d;

    invoke-virtual {p0, p1}, LDR0/f;->C0(LER0/d;)V

    return-void
.end method

.method public final w0()Ly5/a;
    .locals 0

    iget-object p0, p0, LDR0/f;->A:LQ01/M1;

    return-object p0
.end method

.method public final x0()[LLv0/h;
    .locals 0

    iget-object p0, p0, LDR0/f;->B:[LLv0/h;

    return-object p0
.end method

.method public final y0(LER0/c;)V
    .locals 2

    check-cast p1, LER0/d;

    iget-object p0, p0, LDR0/f;->A:LQ01/M1;

    iget-object v0, p0, LQ01/M1;->d:Landroid/widget/ImageView;

    invoke-static {v0}, Lcom/bumptech/glide/b;->f(Landroid/view/View;)Lcom/bumptech/glide/m;

    move-result-object v0

    iget-object v1, p1, LER0/d;->d:Ljava/lang/String;

    invoke-virtual {v0, v1}, Lcom/bumptech/glide/m;->w(Ljava/lang/String;)Lcom/bumptech/glide/l;

    move-result-object v0

    invoke-virtual {v0}, Lr7/a;->e()Lr7/a;

    move-result-object v0

    const-string v1, "circleCrop(...)"

    invoke-static {v0, v1}, Lkotlin/jvm/internal/n;->g(Ljava/lang/Object;Ljava/lang/String;)V

    check-cast v0, Lcom/bumptech/glide/l;

    iget-object v1, p1, LkQ0/e;->a:Ljava/lang/String;

    invoke-static {v1}, LdQ0/e;->a(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    iget-object p1, p1, LER0/d;->f:Ljava/lang/String;

    filled-new-array {v1, p1}, [Ljava/lang/String;

    move-result-object p1

    invoke-static {v0, p1}, LdQ0/a;->d(Lcom/bumptech/glide/l;[Ljava/lang/String;)Lcom/bumptech/glide/l;

    move-result-object p1

    iget-object p0, p0, LQ01/M1;->d:Landroid/widget/ImageView;

    invoke-virtual {p1, p0}, Lcom/bumptech/glide/l;->W(Landroid/widget/ImageView;)Ls7/j;

    return-void
.end method
